...
class multiclassLoss():
    ...
    def __call__(self, preds, targs,cen=None):
        ...
        #loss = ( arteryWeight*self.logitsLoss(torch.mul(preds[:,0],preds[:,1]), target_artery) +
        #        veinWeight*self.logitsLoss(torch.mul(preds[:,2],preds[:,1]), target_vein) +
        #         vesselWeight*self.logitsLoss(preds[:,1], target_all)) / (arteryWeight+veinWeight+vesselWeight)
        #loss = ( arteryWeight*self.logitsLoss(torch.mul(preds[:,0],F.sigmoid(preds[:,1])), target_artery) +
        #        veinWeight*self.logitsLoss(torch.mul(preds[:,2],F.sigmoid(preds[:,1])), target_vein) +
        #         vesselWeight*self.logitsLoss(preds[:,1], target_all)) / (arteryWeight+veinWeight+vesselWeight)
...
class multidiceLoss():
    ...
    def __call__(self, preds, targs,step=0,opt=None):
        ...
        #if step>opt.max_step//3:
        loss = (arteryWeight*self.logitsdiceLoss(preds[:,0], target_artery) +
            veinWeight*self.logitsdiceLoss(preds[:,2], target_vein) +
                vesselWeight*self.logitsdiceLoss(preds[:,1], target_all)) / (arteryWeight+veinWeight+vesselWeight)
        # loss = (self.logitsdiceLoss(preds[:,0], target_artery) +
        #     self.logitsdiceLoss(preds[:,2], target_vein) +
        #         self.logitsdiceLoss(preds[:,1], target_all)) / 3

        # else:
        # #if step>opt.max_step//4:
        #     loss =  self.logitsdiceLoss(preds[:, 1], target_all)
        #loss =  self.logitsdiceLoss(preds[:, 1], target_all)
        #else:
        #    loss=0*self.logitsdiceLoss(preds[:, 1], target_all)
        #loss = (self.logitsdiceLoss(preds[:, 0], target_artery) +
               # self.logitsdiceLoss(preds[:, 2], target_vein) ) / 2
