...
def evaluate(data_loader, model, device, task, epoch, mode, num_class):
    ...
    df2 = pd.DataFrame(data=[prediction_list,true_label_onehot_list])
    df2 =df2.T
    df2.columns = ['prediction_list','true_one_hot_list']
    ...
    if mode=='test' or mode=='val':
        print('Sklearn Metrics - Acc: {:.4f} AUC-roc: {:.4f} AUC-pr: {:.4f} F1-score: {:.4f} MCC: {:.4f} SN: {:.4f} SP {:.4f} PRE {:.4f} F1 {:.4f}'.format(acc, auc_roc, auc_pr, F1, mcc,sensitivity,specificity,precision,F1)) 
        results_path = task+'_metrics_{}.csv'.format(mode)
        
        lower_roc, upper_roc, lower_pr, upper_pr = bootstrap_ci(true_label_onehot_list,prediction_list)
        print(f'95ci ROC between {lower_roc:4f} and {upper_roc:.4f} PR between {lower_pr:.4f} and {upper_pr:.4f}')
        with open(results_path,mode='w+',newline='',encoding='utf8') as cfa:
            wf = csv.writer(cfa)
            data2=[[acc,sensitivity,specificity,precision,auc_roc,lower_roc, upper_roc,auc_pr,lower_pr, upper_pr,F1]]
            for i in data2:
                wf.writerow(i)
    ...
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()},auc_roc,auc_pr,acc
...
def bootstrap_ci(y_true,y_scores,n_iterations=1000):
    ...
    return lower_roc, upper_roc, lower_pr, upper_pr