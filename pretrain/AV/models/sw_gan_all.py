...
import os, copy
from opt_all import get_patch_trad_5, modelEvalution
...
class SW_GAN():
    ...
    def setup(self, opt, log_folder):
        ...
        if not self.isTrain or opt.use_pretrained_G:
            ...
            pt_ = {k: v for k, v in pt['model'].items() if k in model_static}
    ...
    def set_input(self, step, train_data=None, train_label_data=None, train_label_data_centerness=None,
        ...
        patch_data, patch_label, label_centerness, data, label = get_patch_trad_5(opt.batch_size,
                                                                                    ...
                                                                                    patch_size2=self.cfg.patch_size_list[1],use_global_semantic=opt.use_global_semantic)
        ...
        if self.use_centerness and self.centerness_map_size[0] == opt.patch_size//2:
            self.label_centerness_map_copy = self.label_centerness_map.clone()
        ...
        if self.use_centerness and self.centerness_map_size[0] == opt.patch_size:
    ...
    def forward_ema(self):
        ...
        if self.use_global_semantic:
            if self.step <= self.cfg.global_warmup_step:
               ...
               #self.update_momentum_ema(self.netG.sn_unet, self.netG.base_layers_global_momentum, 0.99)
    ...
    def backward_D(self, isBackward=True):
        ...
        # for GT
        label_shape = [opt.batch_size*10, 1, pred_real.shape[2]]
        # 0, 1
    ...
    def backward_G(self, isBackward=True):
        ...
        if opt.use_GAN:
            ...
            ones_tensor = torch.ones([opt.batch_size*10, 1, pred_fake.shape[2]])
            if opt.use_cuda:
        ...
        #if self.step>=opt.max_step//3:
        #    opt.lambda_DICE=1
        self.loss_G_DICE = opt.lambda_DICE * self.criterionDICE(self.pre_target, self.patch_label_input,self.step,opt)
