...
class DBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, use_batchnorm=True, attention_type=None):
        ...
        if attention_type == 'scse':
            ...
            self.attention2 = nn.Identity()
        #self.upt = nn.ConvTranspose2d(in_channels,in_channels,kernel_size=2,stride=2)
...
class SegmentationHead(nn.Module):
    def __init__(self, in_channels, num_class, kernel_size=3, upsample=4):
        ...
        #self.upsample2 = nn.UpsamplingBilinear2d(scale_factor=upsample//2) if upsample > 1 else nn.Identity()
        #self.upsample = nn.ConvTranspose2d(in_channels,in_channels,kernel_size=4,stride=4)
...
class PGFusion(nn.Module):
    ...
    def __init__(self, in_channel=384, out_channel=384):
        ...
        self.patch_query = nn.Conv2d(in_channel, in_channel, kernel_size=1,bias=False)
        self.patch_key = nn.Conv2d(in_channel, in_channel, kernel_size=1, bias=False)
        self.patch_value = nn.Conv2d(in_channel, in_channel, kernel_size=1, bias=False)
        self.patch_global_query = nn.Conv2d(in_channel, in_channel, kernel_size=1,bias=False)

        self.global_key = nn.Conv2d(in_channel, in_channel, kernel_size=1,bias=False)
    ...
    def forward(self, patch_rep, global_rep):
        ...
        #out = patch_self_sim_map * ct[0] + patch_global_sim_map * (1 - ct[0])
        out = patch_rep_ + patch_self_sim_map * ct[0] + patch_global_sim_map * (1 - ct[0])
        return out

    def forward_v2(self, patch_rep, global_rep):
        patch_rep_ = patch_rep.clone()
        patch_value = self.patch_value(patch_rep)
        patch_value = patch_value.view(patch_value.size(0), patch_value.size(1), -1)
        patch_key = self.patch_key(patch_rep)
        ...
        patch_query = self.patch_query(patch_rep)
        patch_query = patch_query.view(patch_query.size(0), patch_query.size(1), -1)
        #patch_query_mean = patch_query.mean(2).unsqueeze(2)
        #patch_key_mean = patch_key.mean(2).unsqueeze(2)
        #patch_query = patch_query-patch_query_mean
        #patch_key = patch_key-patch_key_mean
        ...
        #patch_global_query_mean = patch_global_query.mean(2).unsqueeze(2)
        #global_key_mean = global_key.mean(2).unsqueeze(2)

        #patch_global_query = patch_global_query-patch_global_query_mean
        #global_key = global_key-global_key_mean
        ### patch self attention
        patch_self_sim_map = patch_query.transpose(-2,-1) @ patch_key/math.sqrt(dim_k) 
        patch_self_sim_map = self.softmax(patch_self_sim_map)
        patch_self_sim_map = patch_self_sim_map @ patch_value.transpose(-2,-1)
        patch_self_sim_map = patch_self_sim_map.transpose(-2,-1)
        patch_self_sim_map = patch_self_sim_map.view(patch_self_sim_map.size(0), patch_self_sim_map.size(1),
                                                     *patch_rep.size()[2:])
        patch_self_sim_map = self.gamma_patch_self * patch_self_sim_map
        #patch_self_sim_map = 1 * patch_self_sim_map
        ### patch global attention
        patch_global_sim_map = patch_global_query.transpose(-2,-1) @ global_key/math.sqrt(dim_k)
        patch_global_sim_map = self.softmax(patch_global_sim_map)
        patch_global_sim_map = patch_global_sim_map @ global_value.transpose(-2,-1)
        patch_global_sim_map = patch_global_sim_map.transpose(-2,-1)
        patch_global_sim_map = patch_global_sim_map.view(patch_global_sim_map.size(0), patch_global_sim_map.size(1),
                                                         *patch_rep.size()[2:])
        ...
        patch_global_sim_weight_map = torch.split(fusion_sim_weight_map, dim=1, split_size_or_sections=self.in_channel)[1]
        patch_global_sim_weight_map = torch.sigmoid(patch_global_sim_weight_map)  # 0-1
...
if __name__ == '__main__':
    x = torch.randn((2, 384, 16, 16)).cuda()
    m = PGFusion()
    print(m)
