...
if __name__ == '__main__':
    ...
    if not os.path.exists(os.path.join(output,train_or_test,'images11')):
        ...
        os.makedirs(os.path.join(output, train_or_test, 'av'))
        os.makedirs(os.path.join(output, train_or_test, 'images'))
    if not os.path.exists(os.path.join(output,train_or_test,'images00')):
        os.makedirs(os.path.join(output,train_or_test,'images00'))
        os.makedirs(os.path.join(output, train_or_test, 'label00'))


    
    for ind,i in enumerate(tqdm(os.listdir(os.path.join(dataset_path,'training','av'))[30:])):
        ind = ind+30
        prefix_i = i.split('.')[0]
        ...
        for av_type_i in av_type:
            ...
            for patch_size_i in patch_size:
                ...
                for i in trange(max_epoch):
                    ...
                    patch_select_image_nve_mean = patch_select_image.copy()
                    patch_select_image_nve_mean[patch_select_ve == 1] = 0
                    r,g,b = int(np.mean(patch_select_image_nve_mean[patch_select_ve != 1,0])), int(np.mean(patch_select_image_nve_mean[patch_select_ve != 1,1])),int(np.mean(patch_select_image_nve_mean[patch_select_ve != 1,2]))
                    ...
                    patch_select_av = av_rgb[y:y + patch_size, x:x + patch_size, :]
                    patch_select_av_nv = patch_select_av.copy()
                    patch_select_av_nv[patch_select_v == 1,2] =0

                    patch_select_av_na = patch_select_av.copy()
                    patch_select_av_na[patch_select_a == 1,0] = 0
                    ...
                    if not check_overlap([x,y], patchs,patch_size=patch_size_i):
                        ...
                        if av_type_i == 0:
                            ...
                            Image.fromarray(patch_select_image).save(os.path.join(output, train_or_test, 'images',
                                                                                     f'{dataset_name}_a_{ind}_{ind * max_epoch + i}_{patch_size}_{av_type_i}.png'))

                            Image.fromarray(patch_select_av).save(os.path.join(output, train_or_test, 'label10',
                                                                                  f'{dataset_name}_a_{ind}_{ind * max_epoch + i}_{patch_size}_{av_type_i}.png'))

                            Image.fromarray(patch_select_av).save(os.path.join(output, train_or_test, 'av',
                                                                                  f'{dataset_name}_a_{ind}_{ind * max_epoch + i}_{patch_size}_{av_type_i}.png'))
                        elif av_type_i == 1:
                            ...
                            Image.fromarray(patch_select_image_nv).save(os.path.join(output, train_or_test, 'images10',
                                                                                     f'{dataset_name}_a_{ind}_{ind * max_epoch + i}_{patch_size}_{av_type_i}.png'))

                            Image.fromarray(patch_select_image_nv).save(os.path.join(output, train_or_test, 'images',
                                                                                     f'{dataset_name}_a_{ind}_{ind * max_epoch + i}_{patch_size}_{av_type_i}.png'))
                            Image.fromarray(patch_select_image_na).save(os.path.join(output, train_or_test, 'images01',
                                                                                    f'{dataset_name}_v_{ind}_{ind * max_epoch + i}_{patch_size}_{av_type_i}.png'))
                            Image.fromarray(patch_select_image_na).save(os.path.join(output, train_or_test, 'images',
                                                                                     f'{dataset_name}_v_{ind}_{ind * max_epoch + i}_{patch_size}_{av_type_i}.png'))

                            # Image.fromarray(patch_select_image_nve).save(os.path.join(output, train_or_test, 'images00',
                            #                                                   f'{dataset_name}_b_{ind}_{ind * max_epoch + i}_{patch_size}_{av_type_i}.png'))

                            Image.fromarray(patch_select_av_na).save(os.path.join(output, train_or_test, 'av',
                                                                                     f'{dataset_name}_v_{ind}_{ind * max_epoch + i}_{patch_size}_{av_type_i}.png'))


                            Image.fromarray(patch_select_av_nv).save(os.path.join(output, train_or_test, 'av',
                                                                                     f'{dataset_name}_a_{ind}_{ind * max_epoch + i}_{patch_size}_{av_type_i}.png'))
