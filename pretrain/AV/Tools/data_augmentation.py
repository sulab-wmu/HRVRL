...
def data_augmentation1_5(*args):
    ...
    args = tl.prepro.rotation_multi(args, rg=90, is_random=True,
                                                        fill_mode='constant')
...
def data_augmentation4_5(*args):
    ...
    args = tl.prepro.brightness_multi(args, gamma=0.2, gain=1,
                                                                 is_random=True)
    args = np.squeeze(args).astype(np.float32)
...
def data_augmentation2_5(*args):
    # image3 = np.expand_dims(image3,-1)
    args = tl.prepro.zoom_multi(args, zoom_range=[0.7, 1.2], is_random=True,
                                                    fill_mode='constant')
                                                    #fill_mode='reflect')
    args = np.squeeze(args).astype(np.float32)
