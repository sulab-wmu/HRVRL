...
def patchJudge(mask, a, v, av, y, x, patch_h, patch_w, limit=100):
    '''
    :param mask: mask
    :param a: artery
    :param v: vein
    ...
    if np.sum(a[y:y + patch_h, x:x + patch_w]) >= limit and np.sum(v[y:y + patch_h, x:x + patch_w]) == 0:
        return True,0
    elif np.sum(v[y:y + patch_h, x:x + patch_w]) >= limit and np.sum(a[y:y + patch_h, x:x + patch_w]) == 0:
        return True,1
    elif np.sum(av[y:y + patch_h, x:x + patch_w]) >= 2*limit  and np.sum(a[y:y + patch_h, x:x + patch_w])/np.sum(v[y:y + patch_h, x:x + patch_w])<4 and np.sum(v[y:y + patch_h, x:x + patch_w])/np.sum(a[y:y + patch_h, x:x + patch_w])<4:
        return True,2
    elif np.sum(av[y:y + patch_h, x:x + patch_w])==0:
        return True, 3
    else:
        return False,1000
...
if __name__ == '__main__':
    ...
    parser.add_argument('--av_type', type=list, default=[2], help='av_type range from 0 to 2')
    parser.add_argument('--patch_size', nargs='+',type=int, default=[96,128,256,384,512], help='patch_size')
    ...
    if train_or_test=='training':
        path_list = os.listdir(os.path.join(dataset_path,'training','av'))[:]
    else:
        path_list = os.listdir(os.path.join(dataset_path,'test','av'))[:]
    print(f'{len(path_list)}================{path_list}============')
    av_path_list = []
    patch_x_y_list=[]
    patch_list_df = []
    ...
    for ind,i in enumerate(tqdm(path_list[:])):
        ...
        if train_or_test=='training':
            ...
            ind = ind+test_ind 
        prefix_i = i.split('.')[0]
        if os.path.exists(os.path.join(dataset_path,f'{train_or_test}','images',prefix_i+'.tif')) :
            imagepath = os.path.join(dataset_path,f'{train_or_test}','images',prefix_i+'.tif')
        elif os.path.exists(os.path.join(dataset_path,f'{train_or_test}','images',prefix_i+'.jpg')):
            imagepath = os.path.join(dataset_path,f'{train_or_test}','images',prefix_i+'.jpg')
        elif os.path.exists(os.path.join(dataset_path,f'{train_or_test}','images',prefix_i+'.png')):
            imagepath = os.path.join(dataset_path,f'{train_or_test}','images',prefix_i+'.png')
        
        
        av_path = os.path.join(dataset_path,f'{train_or_test}','av',i)
    ...
    df = pd.DataFrame({'av_path_list':av_path_list,'patch_size':patch_list_df,'x_y':patch_x_y_list})
    df.to_csv(f'{output}/{dataset_name}_{train_or_test}_patch_location_v2.csv',index=None)
